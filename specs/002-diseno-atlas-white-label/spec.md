...
- Venta Max IA es genérico: cada instancia lo opera un negocio distinto → la marca
  visible (nombre + acento) debe ser suya, no "Venta Max IA".
...
4. Filtros: "Todas" y "No leídas" con conteos (Venta Max IA no tiene asignación de
...
**Aceptación**:
1. Nueva pestaña Configuración → Marca: input de nombre (default "Venta Max IA") y
...
4. Sin configurar → defaults (nombre "Venta Max IA", acento azul acero `#3f5972`).
...
Venta Max IA), logo por imagen (v1: inicial sobre acento), densidad configurable.
