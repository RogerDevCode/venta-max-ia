...
const globalForSignup = globalThis as unknown as {
  __ventaMaxIaInternalSignup?: AsyncLocalStorage<boolean>;
};
...
function internalSignupContext(): AsyncLocalStorage<boolean> {
  if (!globalForSignup.__ventaMaxIaInternalSignup) {
    globalForSignup.__ventaMaxIaInternalSignup = new AsyncLocalStorage<boolean>();
  }
  return globalForSignup.__ventaMaxIaInternalSignup;
}
...
const globalForAuth = globalThis as unknown as { __ventaMaxIaAuth?: Auth };

export function getAuth(): Auth {
  if (!globalForAuth.__ventaMaxIaAuth) globalForAuth.__ventaMaxIaAuth = createAuth();
  return globalForAuth.__ventaMaxIaAuth;
}
