...
const globalForBus = globalThis as unknown as { __ventaMaxIaBus?: EventEmitter };

function getBus(): EventEmitter {
  if (!globalForBus.__ventaMaxIaBus) {
    const bus = new EventEmitter();
    bus.setMaxListeners(200);
    globalForBus.__ventaMaxIaBus = bus;
  }
  return globalForBus.__ventaMaxIaBus;
}
