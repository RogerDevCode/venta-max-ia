...
export function CatalogClient() {
  ...
  function reviewText() {
    setMessage("Corrector activo: revisa las palabras subrayadas por tu navegador y acepta sólo las sugerencias que correspondan a tu catálogo.");
  }
  ...
  return <div className="max-w-4xl space-y-6">
    ...
    <Card><CardHeader><CardTitle>Categorías ({categories.length} de 9)</CardTitle></CardHeader><CardContent className="space-y-3">
      <div data-testid="category-form" className="flex flex-wrap gap-2"><Input aria-label="Nombre de categoría" lang="es" spellCheck value={categoryName} onChange={(e) => setCategoryName(e.target.value)} /><Button variant="outline" onClick={reviewText}>Revisar texto</Button><Button onClick={() => void createCategory()} disabled={!categoryName.trim() || categories.length >= 9}>Crear categoría</Button></div>
    ...
    <Card><CardHeader><CardTitle>Productos</CardTitle></CardHeader><CardContent className="space-y-3">
      <div data-testid="product-form" className="grid gap-2 md:grid-cols-6"><Input aria-label="SKU" placeholder="SKU (opcional)" value={product.sku} onChange={(e) => setProduct({ ...product, sku: e.target.value })}/><Input aria-label="Nombre de producto" lang="es" spellCheck placeholder="Nombre" value={product.name} onChange={(e) => setProduct({ ...product, name: e.target.value })}/><Input aria-label="Presentación" lang="es" spellCheck placeholder="Presentación (ej. 500 ml)" value={product.description} onChange={(e) => setProduct({ ...product, description: e.target.value })}/><Input aria-label="Precio en centavos" type="number" value={product.price} onChange={(e) => setProduct({ ...product, price: e.target.value })}/><Input aria-label="Stock" type="number" value={product.stock} onChange={(e) => setProduct({ ...product, stock: e.target.value })}/><select data-testid="product-category-select" aria-label="Categoría" value={product.categoryId} onChange={(e) => setProduct({ ...product, categoryId: e.target.value })} className="rounded-md border bg-card px-2">{categories.map((c) => <option key={c.id} value={c.id}>{c.name}</option>)}</select></div>
      <div className="flex gap-2"><Button variant="outline" onClick={reviewText}>Revisar texto</Button><Button onClick={() => void createProduct()} disabled={!product.name.trim() || !product.categoryId}>Crear producto</Button></div>
