...
type Product = { id: string; sku: string; name: string; description: string | null; price: number; stock: number; active: boolean; categoryId: string };
...
export function CatalogClient() {
  ...
  const [product, setProduct] = useState({ sku: "", name: "", price: "0", stock: "0", categoryId: "", active: true });
  ...
  async function createProduct() {
    ...
    setProduct((v) => ({ ...v, sku: "", name: "", price: "0", stock: "0" })); await refetch();
  }
  ...
  async function editProduct(item: Product) {
    ...
    const res = await fetch(`/api/catalog/products/${item.id}`, { method: "PATCH", headers: { "content-type": "application/json" }, body: JSON.stringify({ sku: item.sku, name, description: item.description, price: item.price, stock: item.stock, active: item.active, categoryId: item.categoryId }) });
    if (!res.ok) { setMessage(await errorMessage(res)); return; } await refetch();
  }
  return <div className="max-w-4xl space-y-6">
    ...
    <Card><CardHeader><CardTitle>Categorías ({categories.length} de 9)</CardTitle></CardHeader><CardContent className="space-y-3">
      ...
      {categories.map((c) => <div key={c.id} className="flex items-center justify-between rounded border p-3" data-testid={c.isGeneral ? "category-general" : undefined}><div><strong>{c.name}</strong>{c.isGeneral && <span className="ml-2 text-xs text-muted-foreground">Respaldo</span>}<p className="text-xs text-muted-foreground">{c.description}</p></div><div className="flex gap-2"><Button variant="outline" disabled={c.isGeneral} onClick={() => void editCategory(c)}>Editar</Button><Button variant="outline" aria-label={`Eliminar ${c.name}`} disabled={c.isGeneral} onClick={() => void removeCategory(c)}>Eliminar</Button></div></div>)}
    </CardContent></Card>
    <Card><CardHeader><CardTitle>Productos</CardTitle></CardHeader><CardContent className="space-y-3">
      <div data-testid="product-form" className="grid gap-2 md:grid-cols-5"><Input aria-label="SKU" placeholder="SKU" value={product.sku} onChange={(e) => setProduct({ ...product, sku: e.target.value })}/><Input aria-label="Nombre de producto" placeholder="Nombre" value={product.name} onChange={(e) => setProduct({ ...product, name: e.target.value })}/><Input aria-label="Precio en centavos" type="number" value={product.price} onChange={(e) => setProduct({ ...product, price: e.target.value })}/><Input aria-label="Stock" type="number" value={product.stock} onChange={(e) => setProduct({ ...product, stock: e.target.value })}/><select data-testid="product-category-select" aria-label="Categoría" value={product.categoryId} onChange={(e) => setProduct({ ...product, categoryId: e.target.value })} className="rounded-md border bg-card px-2">{categories.map((c) => <option key={c.id} value={c.id}>{c.name}</option>)}</select></div>
      <Button onClick={() => void createProduct()} disabled={!product.sku.trim() || !product.name.trim() || !product.categoryId}>Crear producto</Button>
      {products.map((p) => <div key={p.id} className="flex items-center justify-between rounded border p-3"><span>{p.name} ({p.sku}) · ${(p.price / 100).toFixed(2)} · Stock: {p.stock}</span><div className="flex gap-2"><Button variant="outline" onClick={() => void editProduct(p)}>Editar</Button><Button variant="outline" onClick={() => void removeProduct(p.id)}>Eliminar</Button></div></div>)}
    </CardContent></Card>
  </div>;
}
