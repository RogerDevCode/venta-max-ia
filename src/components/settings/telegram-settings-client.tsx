...
export function TelegramSettingsClient() {
  ...
  return <div className="max-w-3xl space-y-6"><Card><CardHeader><CardTitle>Canal Telegram</CardTitle><CardDescription>Este token pertenece exclusivamente a este tenant y se cifra antes de guardarse.</CardDescription></CardHeader><CardContent className="space-y-4">{connection && <p className="rounded border p-3 text-sm">Conectado: @{connection.botUsername ?? connection.botId} · token …{connection.tokenLast4}</p>}<div className="space-y-1.5"><Label htmlFor="telegram-token">Token del bot de este negocio</Label><Input id="telegram-token" type="password" autoComplete="off" value={token} onChange={(e) => setToken(e.target.value)} placeholder="Token entregado por @BotFather" /><p className="text-xs text-muted-foreground">En BotFather: /newbot → copia el token de atención a clientes de este negocio. El bot administrativo se configura por separado en el servidor.</p></div><section aria-label="Panel de incidencias" className="rounded border bg-panel/50 p-3 text-sm"><div className="flex items-center justify-between gap-3"><div><p className="font-medium">Panel de incidencias</p><p role={message ? "alert" : undefined} className="text-muted-foreground">{message ?? "Sin incidencias activas."}</p></div>{message && <Button variant="outline" size="sm" onClick={() => setMessage(null)}>Limpiar</Button>}</div></section><Button disabled={saving || token.trim().length < 20} onClick={() => void save()}>{saving ? "Conectando…" : "Probar, guardar y registrar webhook"}</Button></CardContent></Card></div>;
}
