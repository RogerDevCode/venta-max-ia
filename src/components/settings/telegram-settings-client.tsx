...
export function TelegramSettingsClient() {
  ...
  async function load() { const res = await fetch("/api/settings/telegram"); if (res.ok) setConnection((await res.json() as { connection: Connection | null }).connection); }
  useEffect(() => { void load(); }, []);
  async function save() { setSaving(true); setMessage(null); const res = await fetch("/api/settings/telegram", { method: "PUT", headers: { "content-type": "application/json" }, body: JSON.stringify({ token }) }); setSaving(false); if (!res.ok) { const data = await res.json().catch(() => null) as { error?: { message?: string } } | null; setMessage(data?.error?.message ?? "No se pudo conectar."); return; } setToken(""); setMessage("Bot conectado y webhook registrado."); await load(); }
